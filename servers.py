...
app.config['JWT_SECRET_KEY'] = '4324324324324ffrdawravweraewvraewr'
...
def check_duration():
    ...
    if elapsed_time >= piramid['duration']:
        ...
        mycursor.execute("UPDATE piramid SET balance = minshag, participants = 0, start_date = NOW(), lastuser = DEFAULT, minstavka = DEFAULT, end_date = DATE_ADD(NOW(), INTERVAL duration SECOND) WHERE id = 1")
        conn.commit()
...
def login():
    ...
    if myresult is None:
        ...
        if bcrypt.checkpw(password.encode('utf-8'), myresult['password'].encode('utf-8')):
            ...
            return jsonify({'status': 'success', 'message': 'Авторизация успешна', 'user': myresult, 'token': access_token})
        else:
            return jsonify({'status': 'error', 'message': 'Неверное имя пользователя или пароль'}), 400
...
def deposit():
    ...
    mycursor.execute("SELECT * FROM user where login = %s", (current_user_login,))
    user = mycursor.fetchone()
    
    if not user:
        return jsonify({'status': 'error', 'message': 'Пользователь не найден'})
    
    # Получаем баланс пирамиды
    mycursor.execute("SELECT * FROM piramid WHERE id = 1")
    piramid = mycursor.fetchone()
    piramid_balance = int(piramid['balance'])
    deposit_amount = piramid_balance * 2
    piramid_balancekash = piramid_balance * 1.7
    last_user_login = piramid['lastuser']
    # Проверяем, достаточно ли у пользователя средств
    if deposit_amount > int(user['balance']):
        conn.close()
        return jsonify({'status': 'error', 'message': 'У Вас недостаточно средств для этой операции'})
    
    # Обновляем баланс пользователя
    mycursor.execute("UPDATE user SET balance = balance - %s WHERE login = %s", (deposit_amount, current_user_login))
    mycursor.execute("UPDATE user SET balance = balance + %s WHERE login = %s;", (piramid_balancekash, last_user_login))

    # Обновляем пирамиду
    duration = int(piramid['duration'])
    mycursor.execute("UPDATE piramid SET balance = balance * 2, minstavka = balance * 2, start_date = NOW(), end_date = DATE_ADD(NOW(), INTERVAL %s SECOND), participants = participants + 1, lastuser = %s WHERE id = 1", (duration, current_user_login))
    
    # Получаем обновленный баланс пользователя
    mycursor.execute("SELECT balance FROM user WHERE login = %s", (current_user_login,))
    updated_user = mycursor.fetchone()
    updated_balance = int(updated_user['balance'])
    
    conn.commit()
    conn.close()
    
    return jsonify({'status': 'success', 'message': 'Пирамида успешно пополнена', 'updated_balance': updated_balance})
...
def deposit2():
    ...
    # Обновляем пирамиду
    duration = int(piramid['duration'])
    mycursor.execute("UPDATE piramid SET balance = balance * 2, minstavka = balance * 2, start_date = NOW(), end_date = DATE_ADD(NOW(), INTERVAL %s SECOND), participants = participants + 1, lastuser = %s WHERE id = 2", (duration, current_user_login))
...
def get_user_info():
    ...
    data = request.json
    requested_login = data.get('lname')
    
    conn = pool.get_connection()
    mycursor = conn.cursor(dictionary=True)

    # Запрос в БД
    mycursor.execute("SELECT * FROM user WHERE lname = %s;", (requested_login,))
    user = mycursor.fetchone()
    mycursor.close()
    conn.close()
    ...
    # Если пользователь запрашивает информацию о другом пользователе
    if current_user_login != requested_login:
    ...
    # Удалите поля, которые не следует отправлять в ответе
    del user['hide_email']
    del user['hide_phone']
    del user['hide_city']
