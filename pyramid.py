...
class Root(customtkinter.CTk):
    def __init__(self):
        ...
        self._frame = None
        self.switch_frame(Main_frame,data_updater=self.data_updater,rightbar=self.rightbar)

    def switch_frame(self, frame_class,  data_updater=None, people_updater=None,rightbar=None):
        new_frame = frame_class(root_instance=self, data_updater=self.data_updater, people_updater = self.people_updater,rightbar=self.rightbar)  # замените None на data_updater
    ...
    def make_get_request(self, url):
        ...
        if response.status_code == 200 and response.text:  # Проверьте, что ответ содержит текст
            return response.json()
        else:
            # Верните None или какое-либо другое значение по умолчанию, если ответ пустой или статус не равен 200
            return None
...
class Frame(customtkinter.CTkFrame):
    def __init__(self, master, root_instance,**kw):
        ...
        self.buttons_info = [
            ...
            {"text": "Профиль",  "image" : self.profile_image,"command": lambda: self.root_instance.switch_frame(UserProfile)},
...
class Rightbar(customtkinter.CTkFrame):
    ...
    def login(self):
        ...
        if response.status_code == 400:
            ...
            self.root_instance.token = self.token
    ...
    def logout(self):
        ...
        if self.root_instance._frame is not None:
            self.root_instance._frame.destroy()
        self.root_instance.switch_frame(Main_frame)
        self.root_instance.token = None
    def open_balance_window(self):
        logged_in_login = self.logged_in_username
        token = self.token
        self.balance_window = Balanceup(logged_in_login, token, self)
...
class Main_frame(customtkinter.CTkFrame):
    def __init__(self, root_instance, data_updater,rightbar , people_updater, master=None, **kw):
        super().__init__(master, **kw)
        ...
        self.button_state_p3 = "#f2f2f2" if self.root_instance.auth_user == 1 else "transparent" 
        self.button_state_p4 = 1 if self.root_instance.auth_user == 1 else 0
        self.create_ui()
        self.update_ui()
    ...
    def create_ui(self):
        ...
        self.progressbar = customtkinter.CTkProgressBar(self.pyramid_one, orientation="horizontal",height=10,determinate_speed=0.013,width=300)
        self.progressbar.grid(row=8, column=1, padx=15, pady=(15, 5),sticky="ew",)
        self.progressbar.start()
        self.button_p1 = customtkinter.CTkButton(self.pyramid_one, text="Депозит", command=self.make_deposit_request, state=self.button_state_p1, fg_color=self.button_state_p2, hover_color=HOVER_BTN_COLOR)
        self.button_p1.grid(row=9, column=1,sticky="e",pady=(10,10),padx=15)
        ...
        # self.ost_time = customtkinter.CTkLabel(self.pyramid_one, text="", font=customtkinter.CTkFont(size=15, weight="bold"))
        # self.ost_time.grid(row=7, column=1, padx=0, pady=(10, 5))
        self.progressbar2 = customtkinter.CTkProgressBar(self.pyramid_two, orientation="horizontal",height=10,width=300)
        self.progressbar2.grid(row=8, column=1, padx=15, pady=(15, 5),sticky="ew",)
        self.button_p2 = customtkinter.CTkButton(self.pyramid_two, text="Депозит", command=self.make_deposit_request2, state=self.button_state_p1, fg_color=self.button_state_p2, hover_color=HOVER_BTN_COLOR)
        self.button_p2.grid(row=9, column=1,sticky="e",pady=(10,15),padx=15)

    def make_deposit_request(self):
        # Получите значение из Entry widget

        # Подготовьте данные для отправки
        headers = {
            "Authorization": f"Bearer {self.root_instance.token}"
        }
        response = requests.post(f"{SERVER_URL}/deposit", headers=headers)
        data = response.json()

        # Проверьте ответ от сервера
        if response.status_code == 200 and data['status'] == 'success':
            new_balance = data.get("updated_balance", 0) 
            self.rightbar.update_frame(new_balance)
            CTkMessagebox(title="Успешно", message="Депозит успешно выполнен")
        elif data['status'] == 'error':
            CTkMessagebox(title="Ошибка", message=data['message'])
        else:
            CTkMessagebox(title="Ошибка", message="Неизвестная ошибка:")

    def make_deposit_request2(self):
            # Получите значение из Entry widget
        ...
        response = requests.post(f"{SERVER_URL}/deposit2", headers=headers)
        data = response.json()
    ...
    def update_ui(self):
        ...
        self.logo_Textp5.configure(text=f"Ставка: {self.minstavka} рублей")
        ...
        self.logo_Textp55.configure(text=f"Ставка: {self.minstavka2} рублей")
...
class Title_frame(customtkinter.CTkFrame):
    def __init__(self, root_instance, rightbar, data_updater,people_updater=None, master=None, **kw):
        super().__init__(master, **kw)
        self.data_updater = data_updater 
        self.people_updater = people_updater
        self.root_instance = root_instance
...
class People_frame(ctk.CTkFrame):
    def __init__(self,root_instance, data_updater, rightbar, people_updater, master=None, **kw):
        ...
        self.init_ui()
    def on_profile_button_clicked(self, event):
        """Открывает новый фрейм с данными юзера, когда кликнута кнопка 'Профиль'."""
        user_id = self.table.get_value(event.row, 0)  # Get the user ID from the first column of the selected row
        new_frame = ctk.CTkFrame(self.root_instance)
        new_frame.pack(fill="both", expand=True)

        # Add widgets to the new frame
        label = ctk.CTkLabel(new_frame, text=f"Профиль пользователя {user_id}")
        label.pack(pady=10)

        data = self.people_updater.get_user_data(user_id)

        # Add widgets for each data field
        for field, value in data.items():
            label = ctk.CTkLabel(new_frame, text=f"{field}: {value}")
            label.pack(pady=5)
    ...
    def update_table_data(self, users):
        ...
        self.table = CTkTable(self.scrollframe, row=rows, column=columns, values=table_data)
        self.table.pack(expand=True, fill="both", padx=5, pady=5)

class Deposit_Frame(ctk.CTkFrame):
    def __init__(self, root_instance, rightbar, data_updater, people_updater, master=None, **kw):
        super().__init__(master, **kw)
        self.root_instance = root_instance
        self.configure(corner_radius=0, fg_color="#2b2b2b",)
        self.pack(fill="both", expand=True)
...
class Registration_Frame(ctk.CTkFrame):
    def __init__(self, root_instance, data_updater, people_updater, rightbar, master=None, **kw):
        super().__init__(master, **kw)
        self.root_instance = root_instance
...
class Balanceup(customtkinter.CTkToplevel):
    def __init__(self, login, token, rightbar_instance, *args, **kwargs):
        super().__init__(*args, **kwargs)
    ...
    def clicked(self):
        ...
        if balance == '' or balance == "0":
            ...
            response = requests.post(f"{SERVER_URL}/update_balance", headers=headers, json={"login": nikname, "balance": balance})
            # Обработка ответа
            if response.status_code == 200:
